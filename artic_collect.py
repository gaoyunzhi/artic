...
def run():
    ...
    if not schedule:
        print('artic skip, min_interval: %d minutes' % GAP_MIN)
        return
    channel, page, detail = schedule
    try:
        posts = artic_scraper.get_posts(page, pages=10)
    except Exception as e:
        message = 'artic fetch failed for %s: %s' % (page, e)
        print(message)
    ...
    for post in posts:
        ...
        if matchKey(str(post), blocklist.items()):
            continue
        album = artic_to_album.get(post, detail)
